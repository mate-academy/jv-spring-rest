...
public class MovieSessionDaoImpl extends AbstractDao<MovieSession> implements MovieSessionDao {
    ...
    public MovieSession update(MovieSession movieSession) {
        ...
        try {
            ...
            transaction.commit();
            //return session.get(MovieSession.class, movieSession.getId());
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
                throw new RuntimeException("Can't update MovieSession in DB", e);
            }
        } finally {
            if (session != null) {
                session.close();
    ...
    public void delete(Long id) {
        ...
        try {
            session = sessionFactory.openSession();
            transaction = session.beginTransaction();
            MovieSession fromDb = session.get(MovieSession.class, id);
            session.delete(fromDb);
            transaction.commit();
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
                throw new RuntimeException("Can't delete MovieSession from DB", e);
            }
        } finally {
            if (session != null) {
                session.close();
