...
public class MovieSessionDaoImpl extends AbstractDao<MovieSession> implements MovieSessionDao {
    ...
    public List<MovieSession> findAvailableSessions(Long movieId, LocalDate date) {
        try (Session session = sessionFactory.openSession()) {
            ...
            throw new DataProcessingException(
                    "Can't get available sessions for movie with id: " + movieId + " for date: "
                            + date, e);
        }
    }
    ...
    public MovieSession update(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException(
                    "Can't update a movie session by id: " + movieSession.getId(), e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public MovieSession delete(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException(
                    "Can't remove a movie session by id: " + movieSession.getId(), e);
        } finally {
            if (session != null) {
                session.close();
            }
