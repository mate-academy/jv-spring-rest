...
public class MovieSessionDaoImpl extends AbstractDao<MovieSession> implements MovieSessionDao {
    ...
    public MovieSession update(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't update a movieSession: " + movieSession, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public MovieSession delete(MovieSession movieSession) {
        ...
        try {
            ...
            Query<MovieSession> query = session.createQuery(
                    "DELETE FROM MovieSession ms WHERE ms.id = :id");
            query.setParameter("id", movieSession.getId());
            query.executeUpdate();
            transaction.commit();
            return movieSession;
        } catch (Exception e) {
            ...
            throw new DataProcessingException(
                    "Can't delete a movie session by id: " + movieSession.getId(), e);
        } finally {
            if (session != null) {
                session.close();
            }
