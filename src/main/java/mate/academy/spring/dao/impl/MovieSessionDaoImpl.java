...
public class MovieSessionDaoImpl extends AbstractDao<MovieSession> implements MovieSessionDao {
    ...
    public void remove(long id) {
        ...
        try {
            ...
            session.remove(session.get(MovieSession.class, id));
            transaction.commit();
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
            }
            throw new DataProcessingException("Can't remove movie session from DB by id: " + id, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public MovieSession update(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't update movie session in DB: " + movieSession, e);
        } finally {
            if (session != null) {
                session.close();
            }
