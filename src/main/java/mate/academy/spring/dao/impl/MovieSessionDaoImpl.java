...
public class MovieSessionDaoImpl extends AbstractDao<MovieSession> implements MovieSessionDao {
    ...
    @Override
    public Optional<MovieSession> update(MovieSession newMovieSession) {
        Session session = null;
        Transaction transaction = null;
        try {
            session = sessionFactory.openSession();
            transaction = session.beginTransaction();
            session.update(newMovieSession);
            transaction.commit();
            return Optional.ofNullable(newMovieSession);
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
            }
            throw new DataProcessingException("Cant update movie session " + newMovieSession, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public void delete(Long id) {
        ...
        try {
            ...
            throw new DataProcessingException("Cant delete movie session with id : " + id, e);
        } finally {
            if (session != null) {
                session.close();
            }
