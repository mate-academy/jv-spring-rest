...
public class MovieSessionDaoImpl extends AbstractDao<MovieSession> implements MovieSessionDao {
    ...
    public void delete(Long id) {
        ...
        MovieSession sessionById = get(id).orElseThrow();
        try {
            session = sessionFactory.openSession();
            transaction = session.beginTransaction();
            session.delete(sessionById);
            transaction.commit();
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
            }
            throw new DataProcessingException("Can't delete movie session with this id " + id, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public MovieSession update(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't update this movie session " + movieSession, e);
        } finally {
            if (session != null) {
                session.close();
            }
