...
public class MovieSessionDaoImpl extends AbstractDao<MovieSession> implements MovieSessionDao {
    ...
    public MovieSession update(MovieSession movieSession) {
        ...
        try {
            ...
            throw new DataProcessingException("Can't update movieSession: "
                    + movieSession, e);
        } finally {
            if (session != null) {
                session.close();
    ...
    public void delete(Long id) {
        ...
        try {
            session = sessionFactory.openSession();
            transaction = session.beginTransaction();
            Query<MovieSession> query = session.createQuery("DELETE FROM "
                    + "MovieSession ms WHERE ms.id = :id");
            ...
            throw new DataProcessingException("Can't delete movieSession by id: "
                    + id, e);
        } finally {
            if (session != null) {
                session.close();
